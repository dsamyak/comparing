...
const WORLDS = [
  { id: 0, name: 'Level 1: Cadet', icon: '⭐', color: '#ff4081', desc: 'Numbers 1–1,000' },
  { id: 1, name: 'Level 2: Navigator', icon: '🧭', color: '#4caf50', desc: 'Numbers 1,001–5,000' },
  { id: 2, name: 'Level 3: Commander', icon: '🚀', color: '#673ab7', desc: 'Numbers 5,001–10,000' },
];
  ...
  useEffect(() => {
    if (audioEnabled && q && !worldComplete && !feedback && currentWorld >= 0) {
      const timer = setTimeout(() => {
        narrationRef.current = narrate(playReadQuestion(q.question), true);
      }, 300);
      return () => {
        clearTimeout(timer);
